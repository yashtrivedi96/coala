...
def run_coala(console_printer=None,
    ...
    try:
        ...
        flush_cache = bool(sections['cli'].get(
                'flush_cache',
                sections['default'].get('flush_cache', False)
                if 'default' in sections else False) or
            settings_changed(log_printer, settings_hash))

        cache = None
        if not sections['cli'].get(
                'disable_caching',
                sections['default'].get('disable_caching', False)
                if 'default' in sections else False):
